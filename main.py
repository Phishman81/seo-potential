...
if uploaded_file is not None:
    ...
    if st.button('Run analysis', key='run_analysis_button', key='Run analysis_1')
    ...
    if st.button('Run analysis', key='run_analysis_button', key='Run analysis_2')
    ...
    if st.button('Run analysis', key='run_analysis_button', key='Run analysis_3')
        for i, success_rate in enumerate(success_scenario):
            month_data = calculate_potential_traffic(data, scenario, success_rate, conversion_value)
            all_data = pd.concat([all_data, month_data])
            st.dataframe(all_data)
            ...
            st.markdown(href, unsafe_allow_html=True)
                ...
                # Store month data
    if st.button('Run analysis', key='run_analysis_button', key='Run analysis_4')
        for i, success_rate in enumerate(success_scenario):
            month_data = calculate_potential_traffic(data, scenario, success_rate, conversion_value)
            all_data = pd.concat([all_data, month_data])
            st.dataframe(all_data)
